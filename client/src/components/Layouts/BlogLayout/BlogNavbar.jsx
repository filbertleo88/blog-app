            ...
            <div className="flex items-center gap-3">
              ...
              {isDashboardPage ? (
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-gray-700">Welcome, {currentUser.name.split(" ")[0]}</span>
                </div>
              ) : (
                <div className="flex items-center gap-2 bg-gray-50 rounded-full pl-2 pr-4 py-1 hover:bg-gray-100 transition-colors cursor-pointer group relative">
                  ...
                  <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    ...
                    <div className="p-1">
                      ...
                      <Link to="/blog-posts" className="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-md transition-colors">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path
                            strokeLinecap="round"
          ...
          <nav className="container mx-auto py-4">
            ...
            {currentUser && (
              <div className="border-t border-gray-200 mt-4 pt-4 px-4">
                ...
                <Link to="/blog-posts" className="block py-2 text-gray-700 hover:text-sky-600" onClick={() => setOpenSideMenu(false)}>
                  My Posts
                </Link>
                <button
                  onClick={() => {
...
// const BlogNavbar = ({ activeMenu, onLoginClick, posts = [], user, onLogout }) => {
//   const [openSideMenu, setOpenSideMenu] = useState(false);
//   const [isSearchVisible, setIsSearchVisible] = useState(false);
//   const [searchQuery, setSearchQuery] = useState("");
//   const [currentUser, setCurrentUser] = useState(user);
...
//             {menuItems.map((item, index) => (
//               <Link to={item.path} key={index} className="relative group">
//                 <div className={`text-[15px] font-medium ${activeMenu === item.label ? "text-sky-600" : "text-gray-700 hover:text-sky-500"}`}>
//                   {item.label}
//                   <span className={`absolute inset-x-0 -bottom-1 h-[2px] bg-sky-500 transition-transform duration-300 origin-left ${activeMenu === item.label ? "scale-x-100" : "scale-x-0"} group-hover:scale-x-100`}></span>
//                 </div>
//               </Link>
